...
$(document).ready(() => {
  ...
  function renderTable(dataArray) {
				...
				result += `<tr><td>공급처</td><td>입고날짜</td><td>혈액코드</td><td>혈액형</td><td>종류</td><td>사용확인</td></tr>`
				...
				result += `<tr>
							...
							<td>${v.dt_type}</td>
							<td><button class="req" idx="${v.bd_code}">사용</button></td>
						   </tr>`;
				})
				result += `</table>`;
  ...
  function handleSearch() {
    ...
    const searchDate = dateInput.val().slice(2).replace(/-/g, '/');
    const searchDtType = bloodDtTypeSelect.val();
    const filteredData = data.filter(item => {
      if (searchDate && item.release_day !== searchDate) {
        return false;
      }
  ...
  $(document).on('click', '.req', function(){
	  ...
	  if (confirm(vidx + ' 혈액을 사용처리 하시겠습니까?')) {
    
	  
	  console.log("vidx값은" + vidx);
	  $.ajax({
		  url: `/hos/bloodstock/use.do`,
		  data: { "bdcode" : vidx },
		  type: 'post',
		  dataType: 'text',
		  success: function(res) {
